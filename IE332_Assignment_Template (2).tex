...
Gaussian noise is a type of statistical noise that follows a bell curve, or normal distribution. When in use, noisy pixels are created using the original pixel values in addition to a random Gaussian noise value to alter appearance. Gaussian noise typically arises in images due to electronic circuit noise or sensor noise from illumination of high temperature. It can be used in adversarial attacks to subtly change pixels across the entire image but enough to have the ability to befuddle and image classifier. 
