...
Arc. (2019, May 18). \textit{DeepFool - a simple and accurate method to fool deep neural networks}. Medium. Retrieved April 28, 2023, from https://towardsdatascience.com/deepfool-a-simple-and-accurate-method-to-fool-deep-neural-networks-17e0d0910ac0 

Boesch, G. (2023, January 2). What is adversarial machine learning? attack methods in 2023. viso.ai. Retrieved April 28, 2023, from \textit{https://viso.ai/deep-learning/adversarial-machine-learning/}

Gomes, J. (2018, January 17). Adversarial attacks and defences for Convolutional Neural Networks. Medium. Retrieved April 28, 2023, from \textit{https://medium.com/onfido-tech/adversarial-attacks-and-defences-for-convolutional-neural-networks-66915ece52e7}
...
Knagg, O. (2019, January 6). Know your enemy. Medium. Retrieved April 28, 2023, from \textit {\\https://towardsdatascience .com/know-your-enemy-7f7c5038bdf3#:~:text=adversarially\%20robust\%20model.-,Projected\%20Gradient\%20Descent
\%20(PGD),copy\%20of\%20your\%20model's\%20weights.}

Lendave, V. (2021, September 22). \textit{A guide to different types of noises and image denoising methods}. Analytics India Magazine. https://analyticsindiamag.com/a-guide-to-different-types-of-noises-and-image-denoising-methods/#:~:text=Gaussian\%20noise\%20has\%20a\%20uniform,distribution\%20has\%20a\%20bell
\%20shape. 

Li, S., Huang, G., Xu, X., \& Lu, H. (2022). Query-based black-box attack against medical image segmentation. \textit{Future Generation Computer Systems, 133}, 331-337.

Liu, D. (2022, December 7). \textit{Black-box score-based attacks on images}. KejiTech. Retrieved April 28, 2023, from https://davideliu.com/2020/06/17/black-box-score-based-attacks-on-images/ 
...
Rosebrock, A. (2021, April 17). Adversarial attacks with FGSM (fast gradient sign method). PyImageSearch. Retrieved April 28, 2023, from \textit {https://pyimagesearch.com/2021/03/01/adversarial-attacks-with-fgsm-fast-gradient-sign-method/}

Schroeder, W. (2022, April 26). \textit{Learning machine learning part 2: Attacking white box models}. Medium. Retrieved April 28, 2023, from https://posts.specterops.io/learning-machine-learning-part-2-attacking-white-box-models-1a10bbb4a2ae 
...
\end{center}
\section*{Loop Invariant Analysis}

\paragraph{PGD} \hspace{0pt} \\

The primary loop invariant is that the perturbation of ''x\_adv'' stays within the range of ''epsilon'' for the duration of the loop. This holds true as the perturbation is clipped to the range between negative and positive epsilon (inclusive), which ensures that ''x\_adv'' stays in the allowed range.

\paragraph{Optimize\_Weights} \hspace{0pt} \\

The main loop invariant rely on the weights and the score. For each iteration, the weights vectors must contain non-negative values that sum to 1, ensuring that the weights remain valid throughout the algorithm. Also, the ''best\_score'' and ''best\_weights'' variables are updated whenever a higher score is discovered, ensuring that these variables always represent the highest discovered score throughout the process.
\\
\\
Initialization: Before the loop starts, best\_weights is initialized to initial\_weights, and best\_score is initialized to -Inf. Here, no better weights and scores can be found, so the invariant holds.
\\
\\
Maintenance: During each iteration, the function generates a new set of weights, creates an adversarial image using those weights, and evaluates the attack. If the new score is better than best\_score, the best\_weights and best\_score variables are updated. This ensures that at the beginning of the next iteration, the invariant still holds because the best weights and scores found are stored in those variables.
\\
\\
Termination: When the loop terminates, the invariant holds because best\_weights and best\_score still store the best set of weights and the score found during the iterations. The function then returns the best\_weights, which is the output.
...
\paragraph{FGSM} \hspace{0pt} \\

The with\_gradient function has a runtime complexity of O(m), where m is the number of parameters in the classifier model, which can be weights or biases. The perturbation calculation has a runtime complexity of O(n), where n is the number of pixels in the input image. Thus, the overall runtime complexity is O(m)+O(n).

\paragraph{PGD} \hspace{0pt} \\

The function loss\_function has a runtime complexity of O(m), and similar to FGSM, m is the number of parameters (weights or biases) in the classifier model. The overall complexity is O(40(m+n)), where 40 is the number of loop iterations, n is the number of pixels in the input image, and m is as previously defined.

\paragraph{Gaussian Noise} \hspace{0pt} \\

The rnorm function, array addition, and clipping the values in the noisy image to be between [0,1] all have a runtime complexity of O(n). Returning the noisy image has a runtime complexity of O(1). Therefore, the algorithm as a whole has a runtime complexity of O(n).

\paragraph{Perlin Noise} \hspace{0pt} \\

The dim, vector, and array functions have a complexity of O(n). The perlin noise function has a runtime complexity of O(2\textsuperscript{n}). Array multiplication has a runtime complexity of O(n\textsuperscript{2}) and array addition has a runtime complexity of O(n). Thus, the algorithm has an overall runtime complexity of O(2\textsuperscript{n}).

\paragraph{Random Noise} \hspace{0pt} \\

The runif, pmin, and pmax functions have a runtime complexity of O(n). Returning the noisy image has a runtime complexity of O(1). Thus, the algorithm as a whole has a runtime complexity of O(n).

\paragraph{Weighted Majority Attack} \hspace{0pt} \\

The weighted majority attack combines all of the attack functions, so the run time complexity for this would be the combination of all the attacks, plus clipping the values and combining the results. 
\\
FGSM runtime: O(m) + O(n)
\\
PGD runtime: O(40(m+n))
\\
Gaussian runtime: O(n)
\\
Random noise runtime:O(n)
\\
Clipping values: O(n)
\\
Combining adversarial results: O(n)
\\
In all of these, n is the number of pixels. Combining these, the overall runtime is O(5n + m + 40(m+n))

\paragraph{Evaluate Attack} \hspace{0pt} \\

The evaluate attack evaluates the effectiveness of the attacks.
\\
Get the model's predictions for adversarial image: O(m)
\\
Convert to numeric: O(1)
\\
Find the difference between adversarial and original: O(n) 
\\
Converting image to tensor: O(n)
\\
Evaluation of score: O(n)
\\
The over runtime for the evaluate attack function is O(3n + m), where m is the complexity of the model, and n is the number of pixels of the image.

\paragraph{Optimize weights} \hspace{0pt} \\

The optimize weights function takes in the initialized weights and optimizes them. The runtime would be the combination of the attacks, plus the iterations.
\\
Update weights: O(w)
\\
Normalize weights: O(w)
\\
iterations: O(1)
\\
Weighted majority attack function: O(5n + m + 40(m+n))
\\
Evaluate attack: O(3n + m)
\\
Combining these, the overall runtime is O(1*(2w + 8n + 2m + 40(m+n))). In this case, the number of iterations is 1, but the runtime can be increased based on the number of iterations. W is the length of the weights vector.
...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{|c|c|c|}
    \hline
      Algorithm & Grass Image* Walltime (sec/image) & Dandelion Image* Walltime (sec/image) \\
      \hline
      FGSM & 0.96 & 1.00\\ 
      PGD** & 30 & 30\\
      Gaussian Noise & 0.43 & 0.43\\
      Perlin Noise & 0.25 & 0.23\\
      Random Noise & 0.22 & 0.23\\
      Weighted Majority*** & 120 & 120\\
      \hline
    \end{tabular}
    \\
  \end{center}
...
*Walltime was taken by using all 49 given grass images and all 38 given dandelion images then dividing the time by number of images
\\
**Due to its long runtime, our runtime sample for PGD consisted of 3 images for grass and 3 images for dandelions; these are approximations
\\
***Due to its long runtime, we used the runtime of a single trial through the weighted majority algorithm for both dandelion and grass
...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{|c|c|c|}
    \hline
      Algorithm & Grass Image* Percent Fooled (\%) & Dandelion Image* Percent Fooled (\%) \\
      \hline
      FGSM & 2.0 & 100\\ 
      PGD & 100 & 100\\
...
\setlength{\parindent}{0cm}
*Percent fooled was taken by using all 49 given grass images and all 38 given dandelion images then dividing the number fooled by number of images
\\
**Due to their long runtimes, our sample for PGD and weighted majority consisted of 3 images for grass and 3 images for dandelions; these are approximations
\end{document}
